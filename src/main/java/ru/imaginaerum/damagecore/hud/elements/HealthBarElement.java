...
public class HealthBarElement {
    ...
    public static void render(GuiGraphics gui, float healthPercent) {
        ...
        if (hasAbsorption) {
            ...
            goldW = Math.min(goldW, filledBarW - redW); // не вылезти за край из-за округления

            if (goldW > 0) {
                renderFilledBar(gui, BAR_X + 2 + redW, BAR_Y + 1, goldW, BAR_H - 2, GOLD_TEX_X, GOLD_TEX_Y, GOLD_TEX_W, GOLD_TEX_H, filledBarW - redW, true);
            }
        }
    }
    ...
    static void renderFilledBar(GuiGraphics gui, int barX, int barY, int barW, int barH, int texX, int texY, int texW, int texH, int maxFilledW, boolean isRightSegment) {
        ...
        // Правый край золотой — резервируем место (скос только если сегмент доходит до правого края бара)
